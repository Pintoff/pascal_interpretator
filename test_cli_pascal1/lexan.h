...
public:
    ...
    char c;
    // счетчик считывания массива токенов
    int f_count = 0;
    // конструктор
    ...
    //чтение следующего символа без смещения каретки
    char peek_char(FILE* input_stream) {
        int nextChar = fgetc(input_stream);
        if (nextChar == EOF) {
            return '\0';
        }
        else {
            ungetc(nextChar, input_stream); // Возвращаем символ обратно в поток
    ...
    void is_comment() {
        ...
        if (c == '{') {
            ...
            next_char();
            while (!(c == '*' && peek_char(this->input_stream) == ')')) {
                next_char();
                count++;
                if (c == 10) {
                    flow.line++;
    ...
    void is_opera() {
        ...
        case '*':
            ...
            break;
        case '/':
            if (peek_char(this->input_stream) == '/') {
                is_comment();
            }
            else {
